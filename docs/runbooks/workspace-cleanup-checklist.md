...
## Step 3: Implement The Core Engine (`@repo/core`)
...
## Step 4: Extract the Connectors (`@repo/connectors`)
...
## Step 5: Extract Ingestion (`@repo/ingestion`)
...
## Step 6: Extract Mapping (`@repo/mapping`)
- [ ] Build the normalization layer, transforming the data formats coming from `ingestion` or `BigCommerce` into strictly compliant `@repo/shared` Canonical Interfaces

## Step 7: Build the Database layer (`@repo/db`)
...
- [ ] Scaffold `apps/api` (NestJS @nestjs/graphql) out to manage GraphQL Queries/Mutations and push workloads directly to BullMQ (`@repo/queue`)
...
- [ ] Add error taxonomy logic into `@repo/shared` and Pipeline Error handlers

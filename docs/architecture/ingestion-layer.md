...
The Ingestion layer (`@cdo/ingestion`) handles the acquisition of raw data out of external, proprietary environments and file streams. 

## Separation of Concerns
This layer is **completely isolated** from platform API connectors (`@cdo/connectors`). The ingestion layer has no SDKs for Shopify or commercetools; it focuses purely on parsing raw DOM elements and CSV/JSON bytes.

## Components
1. **Platform Source Connectors (API-Based)**: While APIs typically sit in `@cdo/connectors`, some raw proprietary extractions (e.g. FTP syncs) drop data directly into the ingestion funnel.

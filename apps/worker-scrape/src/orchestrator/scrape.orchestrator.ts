...
export interface ScrapeStrategyConfig {
    jobKind: string;
...
/**
 * ScrapeOrchestrator — the bridge between the Scrape Worker infrastructure and the Core Engine.
 *
 * This behaves identically to the ETL Orchestrator, but specifically manages jobs
 * where the SourceConnector is a headless Playwright scraper.
 */
@Injectable()
export class ScrapeOrchestrator {
    private readonly logger = new Logger(ScrapeOrchestrator.name);
    ...
    async execute(config: ScrapeStrategyConfig): Promise<void> {
